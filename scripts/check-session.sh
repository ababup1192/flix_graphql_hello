#!/usr/bin/env bash
# src/cms のユースケースの pub def のうち、DB に触る（effect に Db / DbRead / DbWrite がある）のに Session を持たない物を集め、
# scripts/session-allowlist.txt（写し・outbox・Runner が主体を決める前に呼ぶ物）と突き合わせる。増えても減っても落ちる。
...
found=$(awk '
    /^mod [A-Za-z]+/ { mod = $2 }
    /^[[:space:]]*pub def / && /\\/ && /(^|[^A-Za-z])Db(Read|Write)?([^A-Za-z]|$)/ && !/(^|[^A-Za-z])Session([^A-Za-z]|$)/ {
        name = $0

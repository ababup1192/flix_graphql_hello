...
DB の Tx は `DbRunner.transact`（Runner とテストの mutate。業務エラー込み）か `DbRunner.withTx`（仕事の tick・プロジェクト slug の解決のような CmsErr 無しの物）を通す。src/ で `withLazyTx` を直に呼べるのは `scripts/tx-allowlist.txt`（DbRunner / TenantTx）だけで、`make check` の `scripts/check-tx.sh` が増減を見張る。業務エラー（CmsErr）のような再開しない effect を `withLazyTx` の外で受けると COMMIT / ROLLBACK と接続の返却が飛び、接続が漏れる（TestTxLeakPg が見張る）。

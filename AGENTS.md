...
権限の証明（`Granted[p]`。`src/cms/model/Granted.flix`）: ContentTypes / Assets / Projects.updateVisibility の pub の入口は `Session.require` を呼ばず、最初の引数で `Granted[ManageTypes]` のような証明を受ける。証明は `Session.grant()`（対象付きは `grantOn(resource)`）だけが作り（中身の `Session.Seal` は非 pub）、印の型 → Permission は trait `Perm` の instance。呼ぶ側（リゾルバ・ContentEngine・MicrocmsImport・テスト）が `ContentTypes.create(Session.grant(), draft)` の形で渡し、作り忘れは引数不足でコンパイルが落ちる。
`Granted[…]` を引数に取る pub は Session を持たなくてよく、check-session.sh がそう扱う。残り（ContentEntries / Webhooks / Schedules / Members / ApiKeys / PreviewTokens）は第 2 段で移す（[docs/design/auth-and-organizations.md](docs/design/auth-and-organizations.md) の「権限の証明（Granted）」）。

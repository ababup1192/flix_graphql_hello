...
query listContentTypes(projectId: Int64) -> many {
    SELECT id, api_id, kind, name, singular, plural, preview_url, icon, created_at, updated_at
...
query insertContentType(projectId: Int64, apiId: String, kind: String, name: String, singular: String, plural: String, previewUrl: String, icon: String) -> one {
    INSERT INTO content_types (project_id, api_id, kind, name, singular, plural, preview_url, icon)
    VALUES (:projectId, :apiId, :kind, :name, :singular, :plural, :previewUrl, :icon)
    RETURNING id
}

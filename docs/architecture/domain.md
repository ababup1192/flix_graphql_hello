...
Auth（UserId / OrgId / Role / Permission / Actor / ApiKeyScope / Visibility）、Webhook（WebhookEvent / DeliveryStatus / Webhook / WebhookDelivery）、Schedule（ScheduleAction / ScheduleStatus / Schedule）、Ulid（配信記録など時刻順の記録の id）、Ids（TypeId / FieldId / ApiId / TypeName）、ContentType（enum・レコード・Draft / Changes・FieldConfig）、Entry（EntryId / Stage / EntryData / IdGen / EntryPage / PageInfo）、Cursor（一覧の位置。orderBy のキーの値 + id を base64url にした不透明な文字列。`pageStart` が after と skip の同時指定と orderBy の不一致を invalid にする。[../design/pagination.md](../design/pagination.md)）、Asset（AssetId / AssetStatus / Upload / AssetPresentation。見せ方は注目点 FocalPoint と切り抜き枠 CropRect の 0〜1 の割合で、画像は触らない。直し方は `Patch`（Keep / Clear / Set）。列は `assets.presentation` の JSONB 1 本で、Json との往復は `AssetPresentation.toJson / fromJson`）、Preloaded（リクエストの間だけ持つ先読みの写しの effect と `Store`。一覧のページの entry id・版・content type。写しはヒントで、無ければ 1 件ずつ引く）

## src/cms/rules/ — 純粋な規則

Authz（役割 → 権限の Datalog。`can` は resource も受ける。read の PAT は viewer に落ちる）、SecretHash（API キー・PAT・プレビュートークンの pepper 付き hash と寿命）、Naming（予約名・衝突・kind と config）、EntryValidation（下書きは緩く、公開は required まで）、EntryLinks（中身から参照を取り出す）、AssetRules（置いてよい mime と大きさ）、AssetPresentationRules（注目点・切り抜き枠の範囲と、Patch の当て方。crop は width / height > 0、left + width と top + height ≤ 1）、RichText（doc の検査・平文・HTML）、Markdown（richText の doc ↔ Markdown。方言は GFM + alerts / `{#id}` / `asset:` / `entry:`。自前のパーサで依存なし）、AssetRefs（中身から asset を取り出す）、EntryDiff（2 つのバージョンの中身の差分。kind ごとの比べ方、many は位置、OBJECT / BLOCKS は再帰、RICH_TEXT は Markdown の行差分 LCS。管理 API の `Entry.diff` の中身）、PublishOrder（複数の entry を公開する順。参照先が先の深さ優先の後順、要求した順は保つ、重複は 1 回。循環と計画の外の未公開の参照先は違反。`publishPlan` / `publishMany` の順を決める）
...
ContentTypes / ContentEntries / Projects / Assets / Accounts / Members / ApiKeys / PersonalTokens / Webhooks / PreviewTokens / Schedules / LinkCards（richText の linkCard の OGP。`fetch` は編集画面が貼った瞬間に管理 API から呼ばれ、`Ogp`（純粋な規則: meta の読み取り、プライベートアドレスの断り、取り直しの判定）で読んで `link_cards` に upsert。監査は積まない）。
...
EntryAutosaves は編集中の中身の**利用者ごとの預かり場**（`entry_autosaves`）。下書き（`entry_contents`）には書かない: 「下書きあり」は `EntryRows.toEntry` が「公開中 かつ 下書きの中身 ≠ 公開の中身」で導くので、自動保存が下書きに触ると公開状態が必ず変わり、ボードのカードが列を移ってチーム全員に「未公開の変更がある」と見える。`entry_contents.stage` に値を足す道は採らない（`Stage` を `entries.stage` と `entry_contents.stage` で使い回していて、`assets.q` の `assetHolders` と `schema_impact.q` の `listFieldValues` が stage で絞っていない。理由は `030_entry_autosaves.sql`）。誰の置き場かは引数に無く、必ず今の主体（`Session.currentUser()` / `currentUserForRead()`）から取り、RLS も自分の行しか見せない。人でない主体（API キー・プレビュー）は置き場を持たない。中身は型の定義で削らず、下書きの検査も通さない（打鍵を落とさない）。下書きと同じ中身になれば置き場を空にする。監査は積まない（[../design/audit-log.md](../design/audit-log.md) の「積まない物」）。掃除はゴミ箱に入れた時の全員分（`031` の policy。**PostgreSQL は DELETE の走査に SELECT の policy も当てる**ので、「DELETE だけプロジェクト全体」では自分の行しか消えない）と、entry の物理削除の FK CASCADE。

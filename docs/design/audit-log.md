...
**`022_audit.sql` の冒頭のコメントは実装より古い。** 「積むのは版の残らない物（メンバー・役割・鍵・型）だけ」と
書いてあるが、実際には `entry.unpublished` と `entry.deleted` も積んでいる。規則は
「**版から読めない物を積む**」で、取り下げ・削除・予約公開の実行はそれに当たる。

WhyNot: このずれを 022 の側で直さない。**当て終わった migration はコメントも含めて変えない**
（checksum がファイル全体に掛かっていて、直すとサーバが起動しなくなる）。
...
| 自動保存（`autosaveEntry`） | 3 秒ごとに来る。編集者 10 人 × 8 時間で 1 日 10 万行になり、業務の変更の記録が埋まる。下書きの保存自体を積んでいない（版が持つ）ので、それより弱い預かり物を積む理由が無い。預かり場は `entry_autosaves` の 1 行だけで、履歴を持たない |

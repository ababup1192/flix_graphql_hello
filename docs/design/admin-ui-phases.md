...
エディタは 2026-09-12 に囲み・折りたたみ・表のセルの中の装飾まで入口が揃い（セルの結合だけは画面に入口が無い）、画面の検査は Vitest のブラウザモード 379 件が `npm run check` と CI で回っている。
済んだフェーズはここから消してある。**残りの正はロードマップの #9**（[roadmap.md](roadmap.md)）、日々の未解決は [2026-09-12-handover.md](2026-09-12-handover.md) の 5 章。
...
- **組織の画面**（`Route.Organization` は `Route.elm` に行があるだけで描く所が無く、組織を切り替えられない）
...
- ダークテーマの詰め、タブレットの並び
- richText の残り: **`+` と `/` が本文の直下（`$from.depth === 1`）でしか出ない**ので、囲みと折りたたみの中でブロックを入れられない。`video` はエディタに入口を作らない決め（YouTube の埋め込みで足りる。CMS 側は素通しで受ける）。書き心地の元は [richtext-note-style.md](richtext-note-style.md)

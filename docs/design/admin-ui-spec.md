...
- **公開は 2 クリック**: レールの「公開」→ dry-run の結果がレールに出る（違反はフォームの項目に赤で写す）→ 同じボタンが「公開」に変わる
...
「〜する」は文末だけでなく、文の中で鉤括弧に入れて呼ぶボタンの名前（「公開は上の帯の『公開する』から。」）も見る（2026-09-12）。
...
確認を出す場所が無くなる。代わりに、押せる・押せないと文字（「公開」/「変更を公開」）で
今どちらに動くかを出す。
...
```
[ タイトル      ] [下書き] [未保存]              [ 下書き保存 ] [ 変更を公開 ]
[ /blogs        ]
```

- 左: 題（型の最初のテキストのフィールド）、API の名前、公開状態の印、保存の様子
- 右: 「下書き保存」（未保存の時だけ押せる。`⌘S` / `Ctrl+S` でも同じ）と公開のボタン
- 公開のボタンの文字は状態で変わる。下書き → 「公開」、公開中 · 下書きあり → 「変更を公開」、公開中で変更なし → 「公開中」（`Editor.publishLabel`）
- **右のレールの案内文にこのボタンの名前を書かない。** 3 通りに変わるので、どの名前を書いても 3 分の 2 は外れる（「公開は上の帯のボタンから。」）
- 公開中で変更が無ければ公開のボタンは押せない（押しても何も変わらない）

### 16.3 公開は保存の後

公開のボタンを押した時に未保存が残っていれば、**先に下書き保存を済ませてから**公開前の確認を開く
（`Editor.pendingPublish`）。人に 2 回押させない。公開されるのは保存済みの下書きなので、
ここを飛ばすと「押したのに古い内容が公開された」が起きる。

確認は書き込まない（`publishCheck`）。結果が出てから、その画面の中で同じボタンをもう一度押す
...
**監査ログとの分担。** コンテンツの編集と、手で押した公開は監査ログには積まない
（版が author を持っていて、二重に持つと片方だけ残る形が作れてしまう。
`src/cms/ContentEntries.flix` の WhyNot）。監査ログに出るコンテンツの出来事は
公開終了・削除・予約公開の 3 つだけなので、監査ログの画面は

- 頭に「コンテンツの編集と公開はここではなく、コンテンツごとの「バージョン履歴」に残ります。」
- 「何を」でコンテンツに絞った時は、出る 3 つを挙げた上で各 API のコンテンツ一覧へのリンク

を出す。断りが無いと**記録が漏れている**ように読める。

...
| 代替テキスト / キャプション / 出典 / 出典の URL / 適用 / 閉じる | richText の image の `alt` / 中身（text と link / bold / italic / strike / code の mark）/ `source` / `sourceUrl`、blockquote の `cite` / `citeUrl` | キャプションは画像の直下の figcaption で、カーソルがある間はキャプションの真上（画像の下端に重なる）に 太字 / 打ち消し / リンク の 3 つの帯が浮く（画像の枠と帯は出ない。見出しと引用は無い）。空で選ぶと placeholder「キャプションを入力」（中央・ラベル無し）。Enter は何もしない、上下の矢印で外の段落へ。旧い `caption` の文字列は読む時だけ中身に写す。代替テキストは画像の上の帯の ALT から（帯が「代替テキストの欄 + 適用 + ×」に入れ替わる）。画像の出典は帯には無い。`source` か `sourceUrl` がある時だけキャプションの下に小さく「出典: …」（URL があればリンク風）と出し、押すと帯の位置に「出典」と「出典の URL」の 2 欄 + 適用 + × が出る（直す・両方空にして消す。新しく付けるのは Markdown / API から）。引用の出典は箱の外の下の右に欄が 1 つだけで、placeholder は「出典を入力」。入れた出典は本文と同じ濃さ（下線も枠も出さない）。URL は出典の文字にリンク（下線、乗せると URL）として掛かるだけで、行に印もボタンも置かない（`citeUrl` を入れるのは当面 Markdown / API から。画面の口は部品の整理の後）。焦点が引用の外にあり両方空なら行ごと出さない（2026-09-11） |
...
| 横に並べる / 1 枚ずつに戻す（画像の横並び） | richText の gallery | 「+」の一覧には入れない。単独の画像が隣り合っている時だけ帯に「横に並べる」が出て、続きの画像を 1 つの gallery に畳む。**列数は選ばせず枚数から決まる**（2 枚なら 2 列、3 枚以上は 3 列で折り返す。`docs/design/richtext-gallery-ui.md` 2）。gallery に帯は無く、枠も面も持たない。並べている画像の帯は 1 枚ずつに戻す / 削除 の 2 つで、押すと gallery が解けて 1 枚ずつの画像に戻る（縮小と配置は並べている間は出さない）。中の画像が 1 枚になっても 0 枚になっても gallery は解ける（2026-09-11）|
...
| 出典を入力 | blockquote の `cite` の placeholder | 引用の箱の外の下、右寄せ（箱との間 18px）。引用に焦点がある間と乗せている間だけ出て、`cite` も `citeUrl` も空のまま離れると畳む（2026-09-11） |
...
| ブロックを追加 | 空の段落の左の「+」 | 一覧は 画像 / 区切り線 / 引用 / コード / 表 / 数式 / 埋め込み。`/` でも開く。**ブロックを入れる口はここだけ**（ツールバーには置かない）。表だけは選んだ後に升目（「大きさを選ぶ」）を出す（2026-09-11） |
| その他の書式 | 固定ツールバーの右寄りの「…」 | ツールバーに出すのは 段落の種類 / 太字 / 斜体 / 打ち消し / コード（文の中）/ 数式（文の中）/ リンク / 箇条書き / 番号付き の 9 個と「…」、右端に 元に戻す / やり直す（`docs/design/toolbar-mock.html` の案 A）。「…」の中は見出し付きの縦の一覧で、「文字」に 下線 / 蛍光ペン / 上付き / 下付き、「ブロック」に チェックリスト / 引用。畳んだ中に押されている物があれば「…」自体に印が付く。Esc と外を押すと閉じる（2026-09-11） |
| 数式（文の中） | 固定ツールバーの `math` mark のボタン | `$…$` を知らなくても掛けられる口。アイコンは Σ（上付き・下付きの道具と見分けが付くため `x²` にしない）。字を選んでいればその字に掛け、選んでいなければ 1 文字（`x`）を数式にして選んだ状態で置く。段落の数式はツールバーに置かず「+」の一覧の「数式」から（ブロックを入れる物は「+」に集める）（2026-09-11） |
...
| 段落を H2 に / 段落を H3 に / 太字 / 打ち消し / コード（文の中） / 引用 / リンク | 選択の下に浮く帯（BubbleMenu） | note の本文の帯と同じ順。見出しの 2 つは段落ごとに効く（部分文字列には掛からない）ので title で「段落を」と伝える。押した状態は aria-pressed。斜体は固定ツールバーだけ、下線・上付き・下付き・蛍光ペンはその「…」の中だけ |
...
| 動詞の語彙 | 作成 / 追加 / 削除 / 変更 / 修正 / 入力 / 選択 / 検索 / 表示 / 取得 / 接続 / 通知 / コピー。和語（作る・足す・消す・変える・直す・入れる・出る・叩く・投げる・繋ぐ・控える）は画面に出さない | AGENTS.md「和語へ言い換えない」 |

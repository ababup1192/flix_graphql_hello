...
| 何を | doc | 備考 |
|---|---|---|
| 画像の出典 | `image.attrs.source`（文字）/ `sourceUrl`（http(s)） | caption は `image.content`（inline）。gallery の中の image も同じ |
...
- 平文（toText）は gallery の中の image だけキャプションを出す（inline を素の文字に）。単独の image は出さない（今まで通り）
...
- href だけの時に figure を作らないのは、中身が img 1 つの figure に意味が無く、サイト側の `figure` の CSS（余白・キャプションの下線）が空の figure にも当たるため。gallery の中は並べる箱として figure が常にあるので、そこに付く
...
- 区切り線は素の `<hr>`。「＊　＊　＊」は CSS の `::before` で出す（モック参照）
...
| 何を | 書き出し | 読み | 根拠 |
|---|---|---|---|
| image の出典 | `![alt](asset:ID){width=W height=H source="出典社" sourceUrl=https://…}` | `{...}` を **key=value の並び**として読む（今は `width= height=` の固定形で、他の key が混ざると image でなくなる）。知らない key は捨てる | 直後の行に `— 出典` を置く案は次の段落と区別できず、gallery（1 行に複数）に付けられない |
| image のキャプション | 画像の直後の行（空行を挟まない）に `*キャプション*`。中は普通の inline Markdown（`[t](u)`、`**b**`、`~~s~~`、`` `c` ``）。`*` は `\*`。gallery は画像の枚数だけ `*…*` を空白区切りで並べ、無い画像は `**`（どの画像にも無ければ行自体が無い） | 直後の行が `*` で始まり `*` で終わる時だけキャプション。直前に空行があれば普通の段落（斜体）。gallery の `*…*` の閉じは「後ろが空白 + `*` か行末」の `*`（`***太字***` の中で切らない）。枚数より少なければ前から詰める。**古い `"caption"` の title は読む時だけ受け**、行があればそちらが勝つ | Zenn と同じ形で、GFM の読み手にも斜体の 1 行として見える。title（`"…"`）は文字列しか持てず、リンク付きのキャプションが落ちる。gallery で 1 行 1 キャプションにすると何枚目の物か読めない |
| blockquote の出典 | 引用の最後に空行を挟んで `> — [出典](URL)`（URL 無しは `> — 出典`） | 引用の最後のブロックが `— ` で始まる段落なら cite に畳む | `> 引用\n> — 出典`（空行無し）は 1 段落に繋がるので、書き出しは必ず空行を挟む |
...
- 複数枚のドロップは今まで通り gallery。gallery の中でも同じ 4 欄

### 6.5 引用

- **今**: `> ` かツールバーで blockquote。出典の置き場が無い
- **こうする**: 引用の右下に薄い「出典を追加」の文字。押すと出典と URL の 2 欄が右寄せで出る。空のままなら doc に書かない。引用の中で Enter を 2 回押すと引用を抜ける（今と同じ）

...
| # | 機能 | 誰に | 時間 | 中身 |
|---|---|---|---|---|
| 11 | 公開計画（まとめて dry-run、一括公開。2026-09-08 に実装済み） | 編集者 | 済み | `publishPlan(ids, withDependencies)` が entry ごとの `publishReport` を参照先が先の順（`steps`。REQUESTED / DEPENDENCY）に並べ、`publishMany(ids, withDependencies)` が同じ順で 1 つの mutation（1 つの Tx）で全部公開する。1 つでも通らなければ何も公開せず INVALID（path は `b1.fields.title` のように entry id 付き）。順は純粋な `PublishOrder`（深さ優先の後順。循環と、`withDependencies: false` の計画の外の参照先は計画の違反）。**microCMS には無い**。残り（同じ時刻にまとめて予約する使い方が出た時か、他社に出す前。半日）: 予約の Release `schedulePublishMany(ids, at)`。1000 件を 1 つの予約にして tick が `publishMany` を 1 回呼ぶ（今は 1 件ずつで 20 件 / 2 秒なので末尾が 2 分後）。予約時に plan で検査して違反なら断り、実行時に再検査。上限 500 件、既定は全部か無しか、`onViolation: SKIP` で違反と下流を外して残りを出す。落ちた時は失敗を書く短い Tx で `release.failed` の Webhook を積む（Facade の外で emit を呼ぶ唯一の例外）|
